    ...
    if (event.key === "Enter" && search.trim()) {
      ...
      if (!searchList.includes(search)) {
        ...
        const response = await axios.post(
          process.env.NEXT_PUBLIC_API_ENPOINT + "/api/auth/keyword/suggestion",
          {
            keyword: search,
          },
          {
      ...
      <div className="mt-4 flex flex-wrap gap-2">
        {searchList.map((tag, index) => (
          <div
            ...
            key={index}
            className="flex cursor-pointer items-center space-x-1 rounded-full bg-gray-100 px-2 py-1 text-xs text-gray-800"
          >
            <span>{tag}</span>
            <button
              onClick={() => handleRemoveTag(index)}
